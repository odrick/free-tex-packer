...
class PackProperties extends React.Component {
    constructor(props) {
        ...
        this.state = { packer: this.packOptions.packer, selectedItem: null };

        Observer.on(GLOBAL_EVENT.FRAME_SET, (selectedItem) => {
            if (!selectedItem)
                return this.setState({ ...this.state, selectedItem: null });
            if (!selectedItem.pivot) selectedItem.pivot = { x: 0.5, y: 0.5 };
            this.setState({ ...this.state, selectedItem: { ...selectedItem } });
        });

        Observer.on(GLOBAL_EVENT.FRAME_UNSET, () => {
    ...
    applyOptionsDefaults(data) {
        ...
        data.removeFileExtension =
            data.removeFileExtension === undefined
                ? false
                : data.removeFileExtension;
        data.prependFolderName =
            data.prependFolderName === undefined
                ? true
                : data.prependFolderName;
        data.scale = data.scale || 1;
        data.filter = getFilterByType(data.filter)
            ? data.filter
            : filters[0].type;
        ...
        data.powerOfTwo =
            data.powerOfTwo === undefined ? false : data.powerOfTwo;
    ...
    updatePackOptions() {
        ...
        data.textureFormat = ReactDOM.findDOMNode(
            this.refs.textureFormat
        ).value;
        ...
        data.base64Export = ReactDOM.findDOMNode(
            this.refs.base64Export
        ).checked;
        ...
        data.padding =
            Number(ReactDOM.findDOMNode(this.refs.padding).value) || 0;
        data.extrude =
            Number(ReactDOM.findDOMNode(this.refs.extrude).value) || 0;
        data.allowRotation = ReactDOM.findDOMNode(
            this.refs.allowRotation
        ).checked;
        ...
        data.alphaThreshold = ReactDOM.findDOMNode(
            this.refs.alphaThreshold
        ).value;
    ...
    refreshPackOptions() {
        ...
        ReactDOM.findDOMNode(this.refs.tinify).checked =
            this.packOptions.tinify;
        ...
        ReactDOM.findDOMNode(this.refs.exporter).value =
            this.packOptions.exporter;
        ReactDOM.findDOMNode(this.refs.fileName).value =
            this.packOptions.fileName;
        ReactDOM.findDOMNode(this.refs.savePath).value =
        ...
        ReactDOM.findDOMNode(this.refs.trimMode).value =
            this.packOptions.trimMode;
    ...
    onExporterChanged() {
        let exporter = getExporterByType(
            ReactDOM.findDOMNode(this.refs.exporter).value
        );
    ...
    render() {
        ...
        const pivotChange = (key) => {
            if (!selectedItem) return () => {};
            return (e) => {
                const selectedItemCloned = {
                    ...selectedItem,
                };

                selectedItemCloned.pivot[key] = e.target.value;
                this.setState({
                    ...this.state,
                    selectedItem: { ...selectedItemCloned },
                });
                Observer.emit(GLOBAL_EVENT.FRAME_UPDATE, selectedItemCloned);
            };
        };

        return (
            <div className="props-list back-white">
                <h1>Pack Properties</h1>
                <div className="pack-properties-containter">
                    <h2>Selected frame properties</h2>
                    <table>
                        {(function (selectedItem) {
                            if (selectedItem) {
                                const x = selectedItem.pivot.x;
                                const y = selectedItem.pivot.y;
                                return (
                                    <tbody>
                                        <tr>
                                            <td>Pivot</td>
                            ...
                            <tr title={I18.f("ALPHA_THRESHOLD_TITLE")}>
                                ...
                                <td>
                                    <input
                                        ...
                                        className="border-color-gray"
                                        defaultValue={
                                            this.packOptions.alphaThreshold
                                        }
                                        min="0"
                                        max="255"
                                        onBlur={this.onPropChanged}
